				...
				if (op != ' ')
				{
					...
					while (index < args.Length)
					{
						...
						if ((op == 'M' || op == 'm') && args.Length >= index + 2)
						{
							...
							subPathStartPoint = new Point(point.X, point.Y); //NEEDED FOR SUBPATHS
							if (op == 'm')
							{
						...
						else if ((op == 'Z' || op == 'z') && args.Length >= index + 1 && p.Operations.Count > 0)
						{
							p.Close();
							p.MoveTo(subPathStartPoint); //NEEDED FOR SUBPATHS
						}
						else {
